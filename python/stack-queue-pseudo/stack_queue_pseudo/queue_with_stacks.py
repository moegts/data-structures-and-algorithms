from stack_queue_pseudo.stacks import Stack


class PseudoQueue :
    def __init__(self):
        self.secand_stack=Stack() ; self.first_stack=Stack()
    ...
    def dequeue(self):
        ...
        if self.first_stack.top :
            ...
            while not stack1.isEmpty(): self.secand_stack.push(stack1.pop())

            poped = self.secand_stack.pop()
            self.front = self.secand_stack.top
            self.first_stack = Stack()
            stack2 = self.secand_stack
            while not stack2.isEmpty(): self.first_stack.push(stack2.pop())
            return poped
