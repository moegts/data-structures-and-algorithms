...
def test_add():
    table = Hash_table(10)
    table.add('mohamd', "khlil")
    assert table.map[table.Gethash('mohamd')].head.value == ("mohamd", 'khlil')
...
def test_handle_collision():
    ...
    assert table.map[table.Gethash('meera')].head.next.value == (
        "meera", 'Bounty cake')
