...
const screenForNames = (arr) => {
  let tester = /^(Mr.|Mrs.|Ms.|Dr.)\s\S/g;
  return arr.filter(i => tester.test(i));
};
...
let biggerThanLuke = (arr) => {
  let array = arr.reduce((res, i) => {
    if (Number(i.mass) > Number(arr[0].mass)) {
      res.push(i.name);
    }
    return res;
  }, []);
...
const sortBy = (property, arr) => {
  return arr.sort((value1, value2) => value1[property] > value2[property] ? 1 : -1);
};
