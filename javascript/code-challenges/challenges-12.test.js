...
const validateWord = (hook) => {
  return (/^[a-zA-Z]{5,10}$/.test(hook));
};
...
const hasNumber = (look) => {
  return (/\d/.test(look)) && (/^[a-zA-Z]/.test(look)) ? true : false;
};
...
const validateEmail = (validateEmail) => {
  let tester = /^\w+\.?\w+@\w+\.(net|com|org)$/ ;
  return tester.test(validateEmail);
};
...
const validatePhoneNumber = (number) => {
  let tester = /^(\((\d{3})\)[\s]?|(\d{3})[\s-]?)(\d{3})[\s-]?(\d{4})$/;
