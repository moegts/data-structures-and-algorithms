...
------------------------------------------------------------------------------------------------ */
// charArray.sort((a, b) => (a.children.length - b.children.length) || 0);

const isNum = (input) => {
  let stringContainsNumber = function(value) {
    return /\d/.test(value);
  };
  let valueInput = stringContainsNumber(input);
  if (input === Number){
    return true;
  }else if (valueInput === true) {
    return true;
  }else{
    return false;
  }
...
const isCapitalized = (str) => {
  ...
  for (let i = 0; i < arrayStr.length; i++) {
    let a = (arrayStr[i].replace(/\W/g, ''));
    if (hasNumbers(a) !== true) {
      withoutNum.push(a);
    }
  }
  let array = [];
  for (let i = 0; i < withoutNum.length; i++) {
    if ((withoutNum[i][0] === withoutNum[i][0].toUpperCase()) && (withoutNum[i].indexOf(',') < 0) && (withoutNum[i].indexOf('.') < 0) && (withoutNum[i].indexOf(':') < 0)) {
      array.push(withoutNum[i]);
    }
  }
  return array;
};
...
const citiesAtoJ = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (((arr[i][0].replace(/A|B|C|D|E|F|G|H|I|J/g,'')).length)<1){
      array.push(arr[i]);
    }
  }
  return array;
