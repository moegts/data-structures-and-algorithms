...
const longestString = (arr) => {
  ...
  return count.reduce((acc, value, index) => {
    return value > count[acc] ? index : acc;
  }, 0);
};
...
const firstLetters = (arr) => {
  let result = [];
  arr.map(item=>result.push(item.charAt(0)));
  return result;
};
...
const standardizePhoneNumbers = (arr) => {
  ...
  let array = [];
  arr.map(elem => {
    array.push(elem.replace(checker, ''));
  });
  return array;
};
...
const onlyOddChars = (str) => {
  let fainal=[...str];
  let string=[];
  fainal.map(i=>fainal.indexOf(i)%2===1&&string.push(i));
  return(string.join(''));
};
...
const allHappy = (arr) => {
  let fainal = arr.map(i => {
    return i.includes(':)')? true : false;
  });
  return fainal.includes(false) ? false : true;
};
/* ------------------------------------------------------------------------------------------------
