...
const allUpperCase = (arr) => {
  let array1 = [];
  arr.forEach(i=>{array1.push(i.toUpperCase());});
  return array1;
};
...
const greeting = (word) => {
  let greeting = word.toUpperCase() + '!';
  return greeting;
};

const speaker = (words, callback) => {
  let array1 = [];
  words.forEach(i=>{array1.push(callback(i));});
  return array1;
};
...
const createList = (availableItems) => {
  ...
  availableItems.forEach((i)=>{
    if(i.available === true){
      availableInventory.push(i.name);
    }
  });
